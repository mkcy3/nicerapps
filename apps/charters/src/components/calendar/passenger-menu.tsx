      ...
      {({ open }) => (
        <div className="flex w-2/3 flex-col py-1 sm:py-5 xl:w-1/2">
          <div className="relative">
            <Listbox.Button className="text-charcoal relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6">
              ...
              <Listbox.Options className="absolute bottom-full z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                {people.map(({ id, name, icon: Icon }) => (
                  <Listbox.Option
                    ...
                    className={({ active }) =>
                      cn(
                        active ? 'bg-indigo-600 text-white' : 'text-charcoal',
